...
export default function ProductsPage() {
  ...
  function handleExport() {
    ...
    const exportData = products.map((product) => ({
        ...
        "Stok Saat Ini": product.inventory?.currentStock ?? 0,
        "Stok Minimum": product.inventory?.minStock ?? 0,
    ...
    <>
      <div className="mb-8">
        ...
        <p className="text-sm text-buddy-text-muted">
          Kelola seluruh produk bisnis Anda
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-buddy-text-muted">
                  Kategori *
                </label>
                <button
                  type="button"
...
function EditProductModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        setError(json.error ?? "Gagal update produk");
        return;
      }
      onSuccess();
    } catch {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-4 h-4 accent-buddy-purple"
            />
            <label htmlFor="isActive" className="text-sm text-buddy-text-main">
              Produk aktif
            </label>
          </div>
          <div className="flex gap-3 pt-2">
